...
clusterLocalOutliers <- function(spe,
    ...
    if (!combine %in% c("union", "intersection")) {
        stop("'combine' must be 'union' or 'intersection'.")
    }
    ...
    if (length(direction) == 1 && !is.null(names(direction))) {
        # Single named element — treat as single direction for all
    ...
    # --- Per-sample, per-cluster processing ---
    for (sid in sample_ids) {
        sample_mask <- sample_vec == sid
        sample_labels <- labels[sample_mask]
        sample_coords <- coords[sample_mask, , drop = FALSE]
        sample_indices <- which(sample_mask)

        clusters_in_sample <- unique(sample_labels)

        for (cl in clusters_in_sample) {
            # Skip artifact clusters
            if (exclude_artifact_clusters &&
                "cluster_artifact_flag" %in% colnames(cd)) {
                cluster_cells <- sample_indices[sample_labels == cl]
                if (length(cluster_cells) > 0 &&
                    all(cd$cluster_artifact_flag[cluster_cells])) {
                    next
                }
            }

            cluster_mask <- sample_labels == cl
            cluster_indices <- sample_indices[cluster_mask]
            n_cluster <- length(cluster_indices)

            if (n_cluster < 3) next  # need at least 3 cells for z-score

            cluster_coords <- sample_coords[cluster_mask, , drop = FALSE]

            # Compute kNN within this cluster
            k_actual <- min(k, n_cluster - 1)
            nn <- BiocNeighbors::findKNN(cluster_coords, k = k_actual,
                                          warn.ties = FALSE)$index

            # Process each metric
            for (m in all_metrics) {
                values <- as.numeric(cd[[m]][cluster_indices])

                # Log-transform if requested and metric is skewed
                ...
                # Flag outliers based on direction and threshold
                outlier_flags <- switch(dir_m,
                    lower = z_scores < -z_threshold,
                    higher = z_scores > z_threshold,
                    both = abs(z_scores) > z_threshold,
                    abs(z_scores) > z_threshold  # default
                )

                # Store results
                cd[[paste0(m, "_zscore")]][cluster_indices] <- z_scores
                cd[[paste0(m, "_outlier")]][cluster_indices] <- outlier_flags
            }
        }
    }
